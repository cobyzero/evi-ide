...
import { IMetricsService } from '../../workbench/contrib/evi/common/metricsService.js';
import { IEviUpdateService } from '../../workbench/contrib/evi/common/eviUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/evi/electron-main/metricsMainService.js';
import { EviMainUpdateService } from '../../workbench/contrib/evi/electron-main/eviUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/evi/electron-main/sendLLMMessageChannel.js';
import { EviSCMService } from '../../workbench/contrib/evi/electron-main/eviSCMMainService.js';
import { IEviSCMService } from '../../workbench/contrib/evi/common/eviSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/evi/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Evi main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(IEviUpdateService, new SyncDescriptor(EviMainUpdateService, undefined, false));
		services.set(IEviSCMService, new SyncDescriptor(EviSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Evi - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('evi-channel-metrics', metricsChannel);

		const voidUpdatesChannel = ProxyChannel.fromService(accessor.get(IEviUpdateService), disposables);
		mainProcessElectronServer.registerChannel('evi-channel-update', voidUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('evi-channel-llmMessage', sendLLMMessageChannel);

		// Evi added this
		const voidSCMChannel = ProxyChannel.fromService(accessor.get(IEviSCMService), disposables);
		mainProcessElectronServer.registerChannel('evi-channel-scm', voidSCMChannel);

		// Evi added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('evi-channel-mcp', mcpChannel);
