...
// The extension this was called from is here - https://github.com/cobyzero/evi-ide/blob/main/extensions/evi/src/extension/extension.ts
...
const MAX_STREAMING_LINES = 10
const STREAM_REFRESH_INTERVAL = 80
...
export class AutocompleteService extends Disposable implements IAutocompleteService {
	...
	static readonly ID = 'evi.autocompleteService'
	...
	async _provideInlineCompletionItems(
		...
		if (cachedAutocompletion && autocompletionMatchup) {
			if (cachedAutocompletion.status === 'finished') {
				...
				if (cachedAutocompletion.insertText) {
					const inlineCompletions = toInlineCompletions({ autocompletionMatchup, autocompletion: cachedAutocompletion, prefixAndSuffix, position })
					return inlineCompletions
				}
				...
				// silently drop errored completions
			}
		...
		const recentSnippetsList = this._contextGatheringService.getCachedSnippets();
		const relevantContext = recentSnippetsList.length > 0
			? recentSnippetsList.map((text) => `${text}`).join('\n-------------------------------\n')
			: ''
		...
		newAutocompletion.llmPromise = new Promise((resolve, reject) => {
			...
			const requestId = this._llmMessageService.sendLLMMessage({
				...
				onText: ({ fullText }) => {
					...
					const numNewlines = (newAutocompletion.insertText.match(/\n|\r\n/g)?.length || 0)
					if (numNewlines > MAX_STREAMING_LINES) {
						const lines = newAutocompletion.insertText.split(/\r?\n/)
						newAutocompletion.insertText = lines.slice(0, MAX_STREAMING_LINES + 1).join(_ln)
					}

					this._triggerInlineCompletionRefresh()
				},
	...
	private _triggerInlineCompletionRefresh(): void {
		const now = Date.now()
		if (now - this._lastStreamRefresh < STREAM_REFRESH_INTERVAL) return
		this._lastStreamRefresh = now

		this._commandService.executeCommand('editor.action.inlineSuggest.trigger');
	}
