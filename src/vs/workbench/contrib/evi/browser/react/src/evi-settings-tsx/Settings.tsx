...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, EviStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/eviSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { EviButtonBgDarken, EviCustomDropdownBox, EviInputBox2, EviSimpleInputBox, EviSwitch } from '../util/inputs.js'
import { useAccessor, useIsDark, useIsOptedOut, useRefreshModelListener, useRefreshModelState, useSettingsState } from '../util/services.js'
import { X, RefreshCw, Loader2, Check, Asterisk, Plus, CheckCircle } from 'lucide-react'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-evi-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<EviButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IEviSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/cobyzero/evi-ide/blob/main/src/vs/workbench/contrib/evi/common/modelCapabilities.ts`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-evi-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-evi-fg-3 hover:text-evi-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-evi-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Evi, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Evi.`
							: `Evi recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<EviSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-evi-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-evi-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-evi-border-2 bg-evi-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-evi-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<EviButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</EviButtonBgDarken>
					<EviButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (EviStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='evi-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='evi-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					{disabled ? null : (
						<div className="w-5 flex items-center justify-center">
							...
							>
								<Plus size={12} className="text-evi-fg-3 opacity-50" />
							</button>
						</div>
					)}
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-evi-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<EviCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-evi-bg-1 text-evi-fg-1 placeholder:text-evi-fg-3 border border-evi-border-2 focus:border-evi-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-evi-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const handleChangeValue = useCallback((newVal: string) => {
		eviSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [eviSettingsService, providerName, settingName]);

	const handleSave = useCallback(() => {
		eviSettingsService.setSettingOfProvider(providerName, settingName, settingValue)
		setShowSaved(true)
		setTimeout(() => setShowSaved(false), 2000)
	}, [eviSettingsService, providerName, settingName, settingValue]);
						...
						className={`
							...
							${settingValue
								? showSaved
									...
									: 'bg-[#0e70c0]/20 text-[#0e70c0] hover:bg-[#0e70c0]/30'
								: 'bg-evi-bg-2 text-evi-fg-4 cursor-not-allowed'}
						`}
					>
...
// 	const needsModel = isProviderNameDisabled(providerName, eviSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const eviSettingsService = accessor.get('IEviSettingsService')

// 	// const { enabled } = eviSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = eviSettingsService.state.settingsOfProvider[providerName].enabled
// 						eviSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [eviSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = eviSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<EviSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				eviSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <EviInputBox2
		...
		initValue={eviSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			eviSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		eviSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [eviSettingsService])

	return <EviCustomDropdownBox
		className='text-xs text-evi-fg-3 bg-evi-bg-1 border border-evi-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={eviSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-evi-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='evi-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Evi automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const eviSettingsService = accessor.get('IEviSettingsService')
	return <div
		className={`text-evi-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { eviSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		eviSettingsService.setGlobalSetting('autoApprove', {
			...eviSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [eviSettingsService, metricsService])

	return <>
		<EviSwitch
			size={size}
			value={eviSettingsState.globalSettings.autoApprove[approvalType] ?? false}
			onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)}
		/>
		<span className="text-evi-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<EviButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const eviSettings = useSettingsState()
	const isOn = eviSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-evi-border-2 bg-evi-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-evi-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-evi-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-evi-bg-2 text-evi-fg-3 rounded-sm text-xs"
							...
							<span className="text-xs text-evi-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-evi-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-evi-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-evi-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-evi-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			dataStr = JSON.stringify(eviSettingsService.state, null, 2)
			downloadName = 'evi-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					eviSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@evi-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-evi-bg-2 hover:bg-evi-bg-2/80 text-evi-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Evi Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-evi-fg-3 mb-2`}>Evi can access any model that you host locally. We automatically detect your local models by default.</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									<h2 className={`text-3xl mb-2`}>Provider</h2>
									<h3 className={`text-evi-fg-3 mb-2`}>Connect your DeepSeek API key to start coding with Evi.</h3>
									<VoidProviderSettings providerNames={['deepseek']} />
								</ErrorBoundary>
							</div>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-evi-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<EviSwitch
																...
																onChange={(newVal) => eviSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-evi-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-evi-fg-3 bg-evi-bg-1 border border-evi-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-evi-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EviSwitch
															...
															onChange={(newVal) => eviSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-evi-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-evi-fg-3 bg-evi-bg-1 border border-evi-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-evi-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EviSwitch
															...
															onChange={(newVal) => eviSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-evi-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<EviSwitch
															...
															onChange={(newVal) => eviSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-evi-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-evi-fg-3 mt-1'>{`Settings that control the visibility of Void suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<EviSwitch
															...
															onChange={(newVal) => eviSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-evi-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-evi-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<EviSwitch
															...
															onChange={(newVal) => eviSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-evi-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-evi-fg-3 bg-evi-bg-1 border border-evi-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-evi-fg-3 mb-4'>{`Transfer your editor settings into Evi.`}</h4>
								...
								<div>
									...
									<h4 className='text-evi-fg-3 mb-4'>{`Transfer Void's settings and chats in and out of Evi.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<EviButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</EviButtonBgDarken>
											<EviButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</EviButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { eviSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<EviButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</EviButtonBgDarken>
											<EviButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</EviButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-evi-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<EviButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</EviButtonBgDarken>
											<EviButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</EviButtonBgDarken>
											<EviButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</EviButtonBgDarken>
											<EviButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</EviButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-evi-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Void running smoothly. You may opt out below. Regardless of this setting, Evi never sees your code, messages, or API keys.</h4>
									...
									<div className='my-2'>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2 my-2'>
												...
												/>
												<span className='text-evi-fg-3 text-xs pointer-events-none'>{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-evi-fg-3 mb-4`}>
										<ChatMarkdownRender inPTag={true} string={`
System instructions to include with all AI requests.
Alternatively, place a \`.evirules\` file in the root of your workspace.
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<EviSwitch
													...
													onChange={(newValue) => {
														eviSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
												<span className='text-evi-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-evi-fg-3 text-xs mt-1'>
											{`When disabled, Evi will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<EviButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</EviButtonBgDarken>
									</div>
