...
import { EviCommandBarProps } from '../../../eviCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	EVI_GOTO_NEXT_DIFF_ACTION_ID,
	EVI_GOTO_PREV_DIFF_ACTION_ID,
	EVI_GOTO_NEXT_URI_ACTION_ID,
	EVI_GOTO_PREV_URI_ACTION_ID,
	EVI_ACCEPT_FILE_ACTION_ID,
	EVI_REJECT_FILE_ACTION_ID,
	EVI_ACCEPT_ALL_DIFFS_ACTION_ID,
	EVI_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const EviCommandBarMain = ({ uri, editor }: EviCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@evi-scope ${isDark ? 'dark' : ''}`}
	>
		<EviCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const EviCommandBar = ({ uri, editor }: EviCommandBarProps) => {
	...
	const voidModelService = accessor.get('IEviModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(EVI_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(EVI_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(EVI_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(EVI_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(EVI_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(EVI_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(EVI_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(EVI_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-evi-bg-2 shadow-md border border-evi-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-evi-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-evi-bg-2 rounded shadow-md border border-evi-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-evi-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-evi-bg-2 rounded shadow-md border border-evi-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-evi-border-2 [&>*:last-child]:border-r-0">
