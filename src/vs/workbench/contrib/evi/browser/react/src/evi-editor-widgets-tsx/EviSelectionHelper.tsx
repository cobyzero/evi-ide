...
import '../styles.css'
import { EVI_CTRL_K_ACTION_ID, EVI_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { EviSelectionHelperProps } from '../../../../../../contrib/evi/browser/voidSelectionHelperWidget.js';
import { EVI_OPEN_SETTINGS_ACTION_ID } from '../../../eviSettingsPane.js';


export const EviSelectionHelperMain = (props: EviSelectionHelperProps) => {
	...
	>
		<EviSelectionHelper {...props} />
	</div>
}



const EviSelectionHelper = ({ rerenderKey }: EviSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(EVI_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(EVI_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-evi-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === EVI_CTRL_L_ACTION_ID || e.commandId === EVI_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(EVI_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(EVI_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(EVI_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-evi-border-3 bg-evi-bg-2
