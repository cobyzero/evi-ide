...
export const EviInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-evi-fg-1 placeholder:text-evi-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-evi-border-3 bg-evi-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-evi-fg-1 bg-evi-bg-2-alt border-b border-evi-border-3 sticky top-0 bg-evi-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-evi-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-evi-bg-2-alt text-evi-fg-1'}
										`}
...
export const EviSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
	...
	return (
		<input
			...
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-evi-bg-1 text-evi-fg-1 placeholder:text-evi-fg-3 border border-evi-border-2 focus:border-evi-border-1
...
export const VoidInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	...
	return <WidgetComponent
		className='
			bg-evi-bg-1
			@@evi-force-child-placeholder-evi-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-evi-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-evi-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-evi-fg-1`}
...
export const EviCheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => void, className?: string }) => {
...
export function EviCustomDropdownBox<T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			<button
				...
				className="flex items-center bg-transparent whitespace-nowrap w-full gap-1"
				onClick={() => setIsOpen(!isOpen)}
			>
				<span className={`truncate text-xs font-medium ${arrowTouchesText ? '' : ''}`}>
					{getOptionDisplayName(selectedOption)}
				</span>
				<svg
					className={`size-3 flex-shrink-0 text-evi-fg-4 transition-transform duration-150 ${isOpen ? 'rotate-180' : ''}`}
					viewBox="0 0 12 12"
					fill="none"
				>
					<path
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-evi-bg-1 border border-evi-border-2 rounded-xl shadow-xl overflow-hidden evi-fade-in"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
				...
				><div className='overflow-auto max-h-80 py-0.5'>
						...
						{options.map((option) => {
							...
							return (
								<div
									key={optionName}
									className={`flex items-center px-2.5 py-1.5 pr-3 cursor-pointer whitespace-nowrap
									evi-transition
									${thisOptionIsSelected ? 'bg-[var(--evi-ring-color)]/15 text-evi-fg-1 font-medium' : 'text-evi-fg-3 hover:bg-[var(--evi-ring-color)]/10 hover:text-evi-fg-2'}
								`}
									onClick={() => {
								...
								>
									...
									<span className='ml-1.5 text-xs'>{optionName}</span>
									{optionDetail && <span className='ml-2 text-[11px] text-evi-fg-4 opacity-70'>{optionDetail}</span>}
								</div>
							)
						})}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-evi-bg-3'>
		<WidgetComponent
...
export const EviButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string }) => {
...
// export const EviCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-evi-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
 ...
 */
export const EviDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	const accessor = useAccessor();
	const languageService = accessor.get('ILanguageService');
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-evi-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-evi-fg-4 text-xs mb-1 px-1">
