...
import { mountEviCommandBar } from './react/out/evi-editor-widgets-tsx/index.js'
...
import { EVI_ACCEPT_DIFF_ACTION_ID, EVI_REJECT_DIFF_ACTION_ID, EVI_GOTO_NEXT_DIFF_ACTION_ID, EVI_GOTO_PREV_DIFF_ACTION_ID, EVI_GOTO_NEXT_URI_ACTION_ID, EVI_GOTO_PREV_URI_ACTION_ID, EVI_ACCEPT_FILE_ACTION_ID, EVI_REJECT_FILE_ACTION_ID, EVI_ACCEPT_ALL_DIFFS_ACTION_ID, EVI_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IEviCommandBarService {
...
export const IEviCommandBarService = createDecorator<IEviCommandBarService>('EviCommandBarService');
...
export class EviCommandBarService extends Disposable implements IEviCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'evi.EviCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._eviModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IEviCommandBarService, EviCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type EviCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountEviCommandBar(root, accessor, { uri, editor } satisfies EviCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies EviCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('eviAcceptDiffAction', 'Evi: Accept Diff'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IEviCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('eviRejectDiffAction', 'Evi: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('eviGoToNextDiffAction', 'Evi: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('eviGoToPrevDiffAction', 'Evi: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('eviGoToNextUriAction', 'Evi: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('eviGoToPrevUriAction', 'Evi: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('eviAcceptFileAction', 'Evi: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('eviRejectFileAction', 'Evi: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('eviAcceptAllDiffsAction', 'Evi: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('eviRejectAllDiffsAction', 'Evi: Reject All Diffs in All Files'),
			keybinding: {
