...
import { EVI_VIEW_CONTAINER_ID, EVI_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { EVI_TOGGLE_SETTINGS_ACTION_ID } from './eviSettingsPane.js';
import { EVI_CTRL_L_ACTION_ID } from './actionIDs.js';
...
const EVI_OPEN_SIDEBAR_ACTION_ID = 'evi.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: EVI_OPEN_SIDEBAR_ACTION_ID, title: localize2('eviOpenSidebar', 'Evi: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(EVI_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('eviCmdL', 'Evi: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(EVI_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(EVI_OPEN_SIDEBAR_ACTION_ID)
		}
...
const EVI_CMD_SHIFT_L_ACTION_ID = 'evi.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: EVI_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', EVI_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(EVI_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'evi.settingsAction',
			title: `Evi's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(EVI_TOGGLE_SETTINGS_ACTION_ID)
	}
})
