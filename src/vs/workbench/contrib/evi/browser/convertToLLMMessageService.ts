...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/eviSettingsTypes.js';
	...
	for (let i = 0; i < messages.length; i += 1) {
		...
		if (currMsg.role !== 'tool') {
			if (currMsg.role === 'assistant' && currMsg.reasoning) {
				newMessages.push({ ...currMsg, reasoning_content: currMsg.reasoning } as OpenAILLMChatMessage)
			} else {
...
const prepareMessages_XML_tools = (messages: SimpleLLMMessage[], supportsAnthropicReasoning: boolean): AnthropicOrOpenAILLMMessage[] => {
	...
	for (let i = 0; i < messages.length; i += 1) {
		...
		if (c.role === 'assistant') {
			...
			const msg: OpenAILLMChatMessage = {
				role: 'assistant',
				content,
				...(c.reasoning ? { reasoning_content: c.reasoning } : {}),
			} as OpenAILLMChatMessage
			llmChatMessages.push(msg as AnthropicOrOpenAILLMMessage)
		}
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .evirules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IEviSettingsService private readonly eviSettingsService: IEviSettingsService,
		@IEviModelService private readonly eviModelService: IEviModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .evirules files from workspace folders
	private _getEviRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let eviRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.evirules')
				const { model } = this.eviModelService.getModel(uri)
				if (!model) continue
				eviRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return eviRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .evirules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.eviSettingsService.state.globalSettings.aiInstructions;
		const eviRulesFileContent = this._getEviRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (eviRulesFileContent) ans.push(eviRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.eviSettingsService.state
		...
		const modelSelectionOptions = this.eviSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.eviSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.eviSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
