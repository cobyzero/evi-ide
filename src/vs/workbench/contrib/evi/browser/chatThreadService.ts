...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/eviSettingsTypes.js';
...
import { EviFileSnapshot } from '../common/editCodeServiceTypes.js';
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Evi)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Evi), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const voidFileSnapshot = checkpointMessage.eviFileSnapshotOfURI ? checkpointMessage.eviFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { voidFileSnapshot, } }

		const userModifiedEviFileSnapshot = fsPath in checkpointMessage.userModifications.eviFileSnapshotOfURI ? checkpointMessage.userModifications.eviFileSnapshotOfURI[fsPath] ?? null : null
		return { voidFileSnapshot: userModifiedEviFileSnapshot ?? voidFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const eviFileSnapshotOfURI: { [fsPath: string]: EviFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._eviModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { voidFileSnapshot: oldEviFileSnapshot } = res
			...
			const voidFileSnapshot = this._editCodeService.getEviFileSnapshot(URI.file(fsPath))
			if (oldEviFileSnapshot === voidFileSnapshot) continue
			eviFileSnapshotOfURI[fsPath] = voidFileSnapshot
		}
		...
		return { eviFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { eviFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			eviFileSnapshotOfURI: eviFileSnapshotOfURI ?? {},
			userModifications: { eviFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getEviFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			eviFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { eviFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.eviFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { eviFileSnapshotOfURI: eviFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					this._editCodeService.restoreEviFileSnapshot(URI.file(fsPath), voidFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._eviModelService.getModelSafe(uri)
