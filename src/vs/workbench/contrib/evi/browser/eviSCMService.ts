...
import { IEviSCMService } from '../common/eviSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IEviSettingsService } from '../common/eviSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/eviSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('eviGenerateCommitMessageService');
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: IEviSCMService
	...
	constructor(
		...
		this.voidSCM = ProxyChannel.toService<IEviSCMService>(mainProcessService.getChannel('evi-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.eviSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.eviSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.eviSettingsService.state.overridesOfModel
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'evi.generateCommitMessageAction',
			title: localize2('eviCommitMessagePrompt', 'Evi: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('eviCommitMessagePromptTooltip', 'Evi: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'evi.loadingGenerateCommitMessageAction',
			title: localize2('eviCommitMessagePromptCancel', 'Evi: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('eviCommitMessagePromptCancelTooltip', 'Evi: Cancel Commit Message Generation'),
