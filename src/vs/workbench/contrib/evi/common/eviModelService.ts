...
export interface IEviModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): EviModelType;
	getModelFromFsPath(fsPath: string): EviModelType;
	getModelSafe(uri: URI): Promise<EviModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IEviModelService = createDecorator<IEviModelService>('voidEviModelService');

class EviModelService extends Disposable implements IEviModelService {
	_serviceBrand: undefined;
	static readonly ID = 'voidEviModelService';
	...
	getModelSafe = async (uri: URI): Promise<EviModelType> => {
...
registerSingleton(IEviModelService, EviModelService, InstantiationType.Eager);
