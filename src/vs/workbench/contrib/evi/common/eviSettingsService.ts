...
import { EVI_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, EviStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './eviSettingsTypes.js';
...
// type RealEviSettings = Exclude<keyof EviSettingsState, '_modelOptions'>
// type EventProp<T extends RealEviSettings = RealEviSettings> = T extends 'globalSettings' ? [T, keyof EviSettingsState[T]] : T | 'all'


export interface IEviSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: EviSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: EviSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: EviStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, EviStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: EviSettingsState): EviSettingsState => {
...
const _validatedModelState = (state: Omit<EviSettingsState, '_modelOptions'>): EviSettingsState => {
...
const defaultState = () => {
	const d: EviSettingsState = {
		settingsOfProvider: deepClone(defaultSettingsOfProvider),
		modelSelectionOfFeature: { 'Chat': { providerName: 'deepseek', modelName: 'deepseek-v4-flash' }, 'Ctrl+K': { providerName: 'deepseek', modelName: 'deepseek-v4-flash' }, 'Autocomplete': { providerName: 'deepseek', modelName: 'deepseek-v4-flash' }, 'Apply': { providerName: 'deepseek', modelName: 'deepseek-v4-flash' }, 'SCM': { providerName: 'deepseek', modelName: 'deepseek-v4-flash' } },
...
export const IEviSettingsService = createDecorator<IEviSettingsService>('EviSettingsService');
class EviSettingsService extends Disposable implements IEviSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: EviSettingsState;
	...
	dangerousSetState = async (newState: EviSettingsState) => {
	...
	private async _readState(): Promise<EviSettingsState> {
		const encryptedState = this._storageService.get(EVI_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(EVI_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: EviSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: EviStatefulModelInfo[] = [
...
registerSingleton(IEviSettingsService, EviSettingsService, InstantiationType.Eager);
