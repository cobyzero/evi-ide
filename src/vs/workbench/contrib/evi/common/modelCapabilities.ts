...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './eviSettingsTypes.js';
...
// ---------------- DEEPSEEK API ----------------
// https://api-docs.deepseek.com/quick_start/pricing (updated 2026-04-24)
// V4 models: 1M context, 384K output, tool calls, JSON, FIM (Flash)
// legacy deepseek-chat/deepseek-reasoner retire 2026-07-24

const deepseekV4Base = {
...
const deepseekModelOptions = {
	...
	// Legacy aliases (retire 2026-07-24): map to V4 Flash
	'deepseek-chat': {
		...deepseekV4Base,
		contextWindow: 1_000_000,
		supportsFIM: true,
...
const deepseekSettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		const lower = modelName.toLowerCase()
		if (lower.includes('deepseek-v4-flash') || lower === 'deepseek-chat') return null
		if (lower.includes('deepseek-v4-pro')) return null
		if (lower.includes('deepseek-reasoner') || lower.includes('deepseek-r1')) return null
		if (lower.includes('deepseek')) {
			return {
				...
				cost: { cache_read: 0.0028, input: 0.14, output: 0.28 },
				modelName,
				recognizedModelName: 'deepseek-v4-flash',
			}
		}
		return null
	},
...
const emptyProviderSettings: VoidStaticProviderInfo = {
	modelOptions: {},
	modelOptionsFallback: () => null,
}

// @ts-ignore - unused variables are kept for reference
const modelSettingsOfProvider: { [providerName in ProviderName]: VoidStaticProviderInfo } = {
	deepseek: deepseekSettings,
	anthropic: emptyProviderSettings,
	openAI: emptyProviderSettings,
	ollama: emptyProviderSettings,
	vLLM: emptyProviderSettings,
	openRouter: emptyProviderSettings,
	openAICompatible: emptyProviderSettings,
	gemini: emptyProviderSettings,
	groq: emptyProviderSettings,
	xAI: emptyProviderSettings,
	mistral: emptyProviderSettings,
	lmStudio: emptyProviderSettings,
	liteLLM: emptyProviderSettings,
	googleVertex: emptyProviderSettings,
	microsoftAzure: emptyProviderSettings,
	awsBedrock: emptyProviderSettings,
} as const
...
// Keep references to prevent noUnusedLocals errors
void [
	openAISettings, xAISettings, geminiSettings,
	mistralSettings, groqSettings, googleVertexSettings,
	microsoftAzureSettings, awsBedrockSettings, vLLMSettings,
	lmStudioSettings, ollamaSettings, openaiCompatible,
	liteLLMSettings, openRouterSettings
];
