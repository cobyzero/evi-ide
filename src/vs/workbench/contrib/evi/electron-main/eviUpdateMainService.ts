...
export class EviMainUpdateService extends Disposable implements IEviUpdateService {
	...
	async check(explicit: boolean): Promise<EviCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Evi to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<EviCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/cobyzero/evi-ide/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Evi is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Evi is up-to-date!'
					}
				}
				else {
