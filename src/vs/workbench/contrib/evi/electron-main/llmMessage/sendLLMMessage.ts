...
import { displayInfoOfProviderName } from '../../common/eviSettingsTypes.js';
import { sendLLMMessageToProviderImplementation } from './sendLLMMessage.impl.js';
import { sendWithOpencode } from './opencodeLLMProvider.js';
...
export const sendLLMMessage = async ({
	...
	const onError: OnError = ({ message: errorMessage, fullError }) => {
		...
		if (errorMessage === 'TypeError: fetch failed')
			errorMessage = `Failed to fetch from ${displayInfoOfProviderName(providerName).title}. This likely means you specified the wrong endpoint in Evi's Settings, or your local model provider like Ollama is powered off.`
	...
	try {
		// OpenCode's OpenAI-compatible adapter now handles reasoning_content for
		// DeepSeek chat models via replayReasoning. Keep DeepSeek's dedicated
		// reasoner models on Evi's native implementation since they have stricter
		// reasoning validation and no tool support. Other models still use OpenCode.
		const isDeepSeekThinkingModel = providerName === 'deepseek' && /reasoner|thinking|r1/i.test(modelName)
		// Set OPENCODE_DISABLED=1 to fall back to the original implementation
		if (!process.env.OPENCODE_DISABLED && !isDeepSeekThinkingModel && messagesType === 'chatMessages') {
			const settings = settingsOfProvider[providerName] as unknown as Record<string, string>;
			await sendWithOpencode({
				onText, onFinalMessage, onError,
				providerName, modelName, settings,
				messages: messages_ as any[],
				separateSystemMessage, mcpTools, chatMode, _setAborter,
			});
			return;
		}
