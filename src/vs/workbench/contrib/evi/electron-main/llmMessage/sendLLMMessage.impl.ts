...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/eviSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://evi.cobyzero.com', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'Evi', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`Evi providerName was invalid: ${providerName}.`)
}


async function _sendDeepSeekFIM({ messages: { prefix, suffix, stopTokens }, onFinalMessage, onError, settingsOfProvider, modelName: modelName_, _setAborter, providerName, overridesOfModel }: SendFIMParams_Internal) {
	const {
		modelName,
		supportsFIM,
	...
	const deepseek = new OpenAI({
		...
		apiKey: settingsOfProvider.deepseek.apiKey,
		dangerouslyAllowBrowser: true,
		...additionalOpenAIPayload,
	})
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	...
	// Parse XML tool calls even when the provider advertises native tools.
	// DeepSeek reasoning models can return the XML/Invoke form despite the
	// OpenAI-compatible tool schema, so skipping this parser leaves the call
	// visible as plain assistant text. Native tool calls remain supported by
	// the existing delta parser below.
	{
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Evi: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Evi if you want the default url).`)
...
export const sendLLMMessageToProviderImplementation = {
	...
	deepseek: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
		sendFIM: (params) => _sendDeepSeekFIM(params),
		list: (params) => _openaiCompatibleList(params),
	},
	groq: {
		sendChat: (params) => _sendOpenAICompatibleChat(params),
