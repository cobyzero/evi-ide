...
import { ChatMode } from '../../common/eviSettingsTypes.js'
...
export const extractXMLToolsWrapper = (
	...
	const dsml = '(?:｜｜DSML｜｜)?'
	const normalizeInvokeEnvelope = (text: string) => text
		.replace(new RegExp(`<${dsml}tool_calls>\\s*([\\s\\S]*?)\\s*<\\/${dsml}tool_calls>`, 'g'), (_whole, body: string) => body.replace(new RegExp(`<${dsml}invoke\\s+name="([^"]+)"[^>]*>([\\s\\S]*?)<\\/${dsml}invoke>`, 'g'), (_invoke, name: string, paramsBody: string) => {
		const params = [...paramsBody.matchAll(new RegExp(`<${dsml}parameter\\s+name="([^"]+)"[^>]*>([\\s\\S]*?)<\\/${dsml}parameter>`, 'g'))].map(match => `<${match[1]}>${decodeXML(match[2])}</${match[1]}>`).join('')
		return `<${name}>${params}</${name}>`
		}))
		// Compatibility with models that describe the same operation as
		// <view_file><path>...</path></view_file>.
		.replace(/<view_file>\s*<path>([\s\S]*?)<\/path>\s*<\/view_file>/g, (_whole, path: string) => `<read_file><file_path>${decodeXML(path)}</file_path></read_file>`)

	let prevFullTextLen = 0
	const newOnText: OnText = (params) => {
		const normalizedFullText = normalizeInvokeEnvelope(params.fullText)
		if (normalizedFullText.length < prevFullTextLen) {
			// The closing </tool_calls> made the envelope normalizable; restart
			// parsing against the normalized cumulative response.
			prevFullTextLen = 0
			fullText = ''
			openToolTagBuffer = ''
			foundOpenTag = null
		...
		const newText = normalizedFullText.substring(prevFullTextLen)
		prevFullTextLen = normalizedFullText.length
